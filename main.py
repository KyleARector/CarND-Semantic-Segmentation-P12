...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    conv_1x1_layer4 = tf.layers.conv2d(vgg_layer4_out, num_classes, 1,
                                       padding='same',
                                       kernel_initializer=tf.random_normal_initializer(stddev=1e-2),                                        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-4))

    output = tf.layers.conv2d_transpose(conv_1x1_layer7, num_classes, 4, 2,
                                        padding='same',
                                        kernel_initializer=tf.random_normal_initializer(stddev=1e-2),                                        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-4))
    ...
    last_layer = tf.layers.conv2d_transpose(output, num_classes, 16, 8,
                                       padding='same',
                                       kernel_initializer=tf.random_normal_initializer(stddev=1e-2),
                                       kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-4))
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    """
    # logits are a 2D tensor with a pixel per row and a column per class
...
def run():
    ...
    with tf.Session() as sess:
        ...
        # Train NN using the train_nn functionA
